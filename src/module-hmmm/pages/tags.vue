<template>
  <div class="container">
    <thehead :text="text" :dialogVisible.sync="dialogVisible" @searchcipline="getTags"></thehead>
    <subjectform :text="text" :list="list" @btnswitch="btnswitch" @open="open" @isvisible="isvisible"></subjectform>
    <increase :dialogVisible.sync="dialogVisible" :text="text" @Thenewdata="Thenewdata"></increase>
    <!-- 分页 -->
    <el-pagination background layout=" ->, prev, pager, next,sizes,jumper " class="pagination" :total="text.counts"
      :page-size.sync="page.pagesize" :current-page.sync="page.page" :page-sizes="[5, 10, 20, 50]"
      @size-change="getTags" @current-change="getTags">
    </el-pagination>
    <!-- -------- -->
    <!-- 修改 -->
    <el-dialog title="提示" :visible.sync="isVisible" width="40%">
      <el-form label-width="80px" :model="Modifythelabel" :rules="tagsrules">
        <el-form-item label="所属学科">
          <el-select v-model="Modifythelabel.subjectID" placeholder="请选择" style="width: 100%">
            <el-option v-for="item in Thelistof" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
